...
class Transformers:
    ...
    def assign_indexes(self, bus):
        ...
        if global_vars.xfmr_model == 1:
            ...
            self.LIaux_r_node = Buses._node_index.__next__()
            self.LIaux_i_node = Buses._node_index.__next__()
            self.LVaux_r_node = Buses._node_index.__next__()
            self.LVaux_i_node = Buses._node_index.__next__()
    ...
    def stamp_dual(self, V, Ylin_val, Ylin_row, Ylin_col, Jlin_val, Jlin_row, idx_Y, idx_J):
        ...
        trcos = self.tr*np.cos(self.ang*np.pi/180.0)
        trsin = self.tr*np.sin(self.ang*np.pi/180.0)
        # I_aux_r leaving Lr_from_node
        idx_Y = stampY(self.LIaux_r_node,self.Lr_from_node, 1, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        # I_aux_i leaving Li_from_node
        idx_Y = stampY(self.LIaux_i_node,self.Li_from_node, 1, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        # Lr_from setpoint
        idx_Y = stampY(self.Lr_from_node, self.LIaux_r_node, -1, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.LIaux_r_node, trcos, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.LIaux_r_node, -trsin, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        # Li_from setpoint
        idx_Y = stampY( self.Li_from_node,self.LIaux_i_node, -1, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.LIaux_i_node, trsin, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.LIaux_i_node, trcos, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        # LVaux_r KCL
        idx_Y = stampY( self.LIaux_r_node,self.LVaux_r_node, -trcos, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LIaux_i_node,self.LVaux_r_node, -trsin, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.LVaux_r_node, self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Lr_to_node,self.LVaux_r_node, -self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.LVaux_r_node, -self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Li_to_node,self.LVaux_r_node, self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.LVaux_r_node, -self.Bsh_raw/2, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        # LVaux_i KCL
        idx_Y = stampY( self.LIaux_r_node,self.LVaux_i_node, trsin, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LIaux_i_node,self.LVaux_i_node, -trcos, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.LVaux_i_node, self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Li_to_node,self.LVaux_i_node, -self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.LVaux_i_node, self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Lr_to_node,self.LVaux_i_node, -self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.LVaux_i_node, self.Bsh_raw/2, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        
        # Vto losses
        idx_Y = stampY( self.Lr_to_node,self.Lr_to_node, self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.Lr_to_node, -self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)            
        idx_Y = stampY( self.Li_to_node,self.Lr_to_node, -self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.Lr_to_node, self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Li_to_node,self.Lr_to_node, self.Bsh_raw/2, Ylin_val, Ylin_row, Ylin_col, idx_Y)

        idx_Y = stampY( self.Li_to_node,self.Li_to_node, self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_i_node,self.Li_to_node, -self.G_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Lr_to_node,self.Li_to_node, self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.LVaux_r_node,self.Li_to_node, -self.B_pu, Ylin_val, Ylin_row, Ylin_col, idx_Y)
        idx_Y = stampY( self.Lr_to_node,self.Li_to_node, -self.Bsh_raw/2, Ylin_val, Ylin_row, Ylin_col, idx_Y)
