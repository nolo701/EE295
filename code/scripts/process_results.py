...
def process_results(v_sol, buses):
    ...
    print("Minimum voltage at bus: {} with {:.3f} p.u @ {:.3f} deg".format(minVBusid,polar[minV_idx,3],polar[minV_idx,4]))
    print("Maximum voltage at bus: {} with {:.3f} p.u @ {:.3f} deg".format(maxVBusid,polar[maxV_idx,3],polar[maxV_idx,4]))
    print("Minimum Angle at bus: {} with {:.3f} p.u @ {:.3f} deg".format(minAngBusid,polar[minAng_idx,3],polar[minAng_idx,4]))
    print("Maximum Angle at bus: {} with {:.3f} p.u @ {:.3f} deg".format(maxAngBusid,polar[maxAng_idx,3],polar[maxAng_idx,4]))
